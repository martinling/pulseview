...
{
	assert(_unit_size == logic->get_unit_size());
	assert((logic->get_data_length() % _unit_size) == 0);

	lock_guard<recursive_mutex> lock(_mutex);

	append_data(logic->get_data_pointer(),
		logic->get_data_length() / _unit_size);
