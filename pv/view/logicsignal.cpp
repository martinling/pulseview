	...
	_trigger_change(NULL)
{
	...
	_colour = SignalColours[channel->get_index() % countof(SignalColours)];

	/* Populate this channel's trigger setting with whatever we
	 * find in the current session trigger, if anything. */
	_trigger_match = nullptr;
	if ((trigger = SigSession::_sr_session->get_trigger()))
		for (auto stage : trigger->get_stages())
			for (auto match : stage->get_matches())
				if (match->get_channel() == _channel)
					_trigger_match = match->get_type();
}
...
void LogicSignal::paint_mid(QPainter &p, int left, int right)
{
	...
	snapshot->get_subsampled_edges(edges,
		...
		samples_per_pixel / Oversampling, _channel->get_index());
	assert(edges.size() >= 2);
